class Parser:
    ...
    def parse_program(self):
        ...
        while not self.check("EOF"):
            b.append(self.parse_statement()); self.skip_newlines()
    ...
    def parse_statement(self):
        ...
        if t == "embark":
            n = self.parse_embark_expr(); self.expect("NEWLINE"); return {"type": "ExprStatement", "expr": n}
        if t == "reward":
            self.advance(); v = self.parse_expr(); self.expect("NEWLINE"); return {"type": "Reword", "value": v}
        if t == "init":
            self.advance(); ns = [self.expect("ID").value]
            while self.check(","):
                self.advance(); ns.append(self.expect("ID").value)
            self.expect("NEWLINE"); return {"type": "Init", "names": ns}
        if t == "quit":
            self.advance(); self.expect("NEWLINE"); return {"type": "Quit"}
        if t == "continue":
            self.advance(); self.expect("NEWLINE"); return {"type": "Continue"}
        if t == "pass":
            self.advance(); self.expect("NEWLINE"); return {"type": "Pass"}
        if t == "raise":
            self.advance(); v = self.parse_expr(); self.expect("NEWLINE"); return {"type": "Raise", "value": v}
        if t == "while":
            self.expect("while"); c = self.parse_expr(); return {"type": "While", "condition": c, "body": self.parse_block()}
    ...
    def parse_var_decl(self, k):
        ...
        if k == "pouch" and v.get("type") != "PouchLiteral":
            raise SyntaxError("The DM expects a pouch to be written as key-value pairs inside { }.")
        self.expect("NEWLINE")
        return {"type": "AbilityDecl" if k == "ability" else "PouchDecl", "name": n, "value": v}

    def parse_vault(self):
        self.expect("vault"); n = self.expect("ID").value; self.expect("="); p = self.parse_expr(); self.expect("NEWLINE")
        return {"type": "VaultDecl", "name": n, "path": p}

    def parse_assignment_or_expr(self):
        s = self.pos
        t = self.parse_target()
        if self.check("="):
            self.advance(); v = self.parse_expr(); self.expect("NEWLINE")
            return {"type": "Assignment", "target": t, "value": v, "operator": "="}
        if self.peek().type in ("+=", "-=", "*=", "/=", "%="):
            op = self.advance().type; r = self.parse_expr(); self.expect("NEWLINE")
            return {"type": "Assignment", "target": t, "value": r, "operator": op}
        self.pos = s
        e = self.parse_expr(); self.expect("NEWLINE")
    ...
    def parse_target(self):
        ...
        while self.check(".") or self.check("["):
            if self.check("."):
                self.advance(); n = {"type": "FieldAccess", "obj": n, "field": self.expect("ID").value}
            else:
    ...
    def parse_narrate(self):
        self.expect("narrate")
        if self.check("("):
            self.advance(); v = self.parse_expr(); self.expect(")")
        else: v = self.parse_expr()
        self.expect("NEWLINE"); return {"type": "Narrate", "value": v}

    def parse_attempt(self):
        self.expect("attempt"); self.expect("("); c = self.parse_expr(); self.expect(")")
        clauses = [{"condition": c, "body": self.parse_block()}]; f = None
        while self.check("or_attempt"):
            self.advance(); self.expect("("); c = self.parse_expr(); self.expect(")")
            clauses.append({"condition": c, "body": self.parse_block()})
        if self.check("fail"):
            self.advance(); f = self.parse_block()
        return {"type": "Attempt", "clauses": clauses, "fail_body": f}

    def parse_adventure(self):
        self.expect("adventure")
        if self.check("("):
            self.advance(); c = self.parse_expr(); self.expect(")")
            return {"type": "Adventure", "count": c, "body": self.parse_block()}
        n = self.expect("ID").value; self.expect("in"); x = self.parse_expr()
        return {"type": "ForParty", "var": n, "args": [x], "body": self.parse_block()}
    ...
    def parse_quest(self):
        ...
        if not self.check(")"):
            ...
            while self.check(","):
                self.advance(); p.append(self.expect("ID").value)
    ...
    def parse_embark_expr(self):
        ...
        if not self.check(")"):
            ...
            while self.check(","):
                self.advance(); a.append(self.parse_expr())
        self.expect(")"); return {"type": "Embark", "name": n, "args": a}

    def parse_homebrew(self):
        self.expect("homebrew"); k = self.expect("ID").value; n = self.expect("ID").value
        self.expect(":"); self.expect("NEWLINE"); self.expect("INDENT"); f = {}; s = {}; e = {}
        while not self.check("DEDENT"):
            if self.check("ID") and self.peek().value == "STATS":
                self.advance(); s, e = self.parse_stats_block()
            else:
                q = self.expect("ID").value; self.expect("="); f[q] = self.parse_expr(); self.expect("NEWLINE")
            self.skip_newlines()
    ...
    def parse_stats_block(self):
        ...
        while not self.check("DEDENT"):
            if self.check("extra"):
                self.advance(); n = self.expect("ID").value; self.expect("="); e[n] = self.parse_expr(); self.expect("NEWLINE")
            else:
                n = self.expect("ID").value; self.expect("="); s[n] = self.parse_expr(); self.expect("NEWLINE")
            self.skip_newlines()
        self.expect("DEDENT"); return s, e

    def parse_global_stats(self):
        self.advance(); s, e = self.parse_stats_block(); return {"type": "GlobalStats", "stats": s, "extra_stats": e}

    def parse_summon(self):
        self.expect("summon"); v = self.expect("ID").value; self.expect("="); n = self.expect("ID").value
        self.expect("("); self.expect(")"); self.expect("NEWLINE")
        return {"type": "SummonDecl", "var_name": v, "template": n}
    ...
    def parse_submit(self):
        ...
        while self.check("consider"):
            self.advance(); n = None
            if self.check("issue"):
                self.advance(); n = self.expect("ID").value
    ...
    def parse_unary(self):
        if self.check("-"):
            self.advance(); return {"type": "UnaryOp", "op": "-", "operand": self.parse_unary()}
        if self.check("+"):
        ...
        while True:
            if self.check("."):
                ...
                if f == "solve" and self.check("."):
                    ...
                    if not c: c = "/_"
                    self.expect("("); d = self.parse_expr(); self.expect(")")
                    n = {"type": "SolveExpr", "instance": n, "mode": m, "comparator": c, "difficulty": d}
    ...
    def parse_force_call(self, i):
        ...
        self.expect(","); v = self.parse_expr(); self.expect(")")
        return {"type": "ForceExpr", "instance": i, "mode": m, "symbol": s.type, "value": v}

    def parse_sway_call(self, i):
        self.expect("("); a = self.parse_expr(); self.expect(","); q = self.parse_shift_spec(); self.expect(")")
        return {"type": "SwayExpr", "instance": i, "address": a, "shift": q}

    def parse_shift_spec(self):
        if self.peek().type in ("+", "-"):
            s = self.advance().type; n = self.expect("NUMBER").value
            return {"type": "ShiftLiteral", "value": n if s == "+" else -n}
        ...
        if self.check("."):
            self.advance(); return {"type": "ShiftStatRef", "stat": n, "mode": self.expect("ID").value}
    ...
    def parse_primary(self):
        ...
        if t.type == "player":
            self.advance(); self.expect("("); p = self.parse_expr(); self.expect(")"); return {"type": "PlayerInput", "prompt": p}
        ...
        if t.type == "ID":
            self.advance(); return {"type": "Identifier", "name": t.value}
