...
class Parser:
    ...
    def expect(self, type_):
        if not self.check(type_):
            tok = self.peek()
            raise SyntaxError(
                f"The DM expected {type_} but found {tok.type} ('{tok.value}') at line {getattr(tok, 'address', tok.line)}."
            )
        return self.advance()
    ...
    def parse_statement(self):
        ...
        raise SyntaxError(f"The DM doesn't know how to handle '{tok.value}' at line {getattr(tok, 'address', tok.line)}.")
    ...
    def parse_reward(self):
        self.expect("reward")
        value = self.parse_expr(); self.expect("NEWLINE")
    ...
    def parse_attempt(self):
        ...
        while self.check("or_attempt"):
            self.advance(); self.expect("("); cond = self.parse_expr(); self.expect(")")
            clauses.append({"condition": cond, "body": self.parse_block()})
        if self.check("fail"):
            self.advance(); fail_body = self.parse_block()
        return {"type": "Attempt", "clauses": clauses, "fail_body": fail_body}

    def parse_adventure(self):
        self.expect("adventure"); self.expect("("); count = self.parse_expr(); self.expect(")")
        return {"type": "Adventure", "count": count, "body": self.parse_block()}

    def parse_quest(self):
        self.expect("quest"); name = self.expect("ID").value; self.expect("(")
        params = []
        if not self.check(")"):
        ...
        self.expect(")")
        return {"type": "QuestDef", "name": name, "params": params, "body": self.parse_block()}

    def parse_embark_expr(self):
        self.expect("embark"); name = self.expect("ID").value; self.expect("(")
        args = []
        if not self.check(")"):
    ...
    def parse_homebrew(self):
        ...
        self.expect(":"); self.expect("NEWLINE"); self.expect("INDENT")
        fields = {}; stats = {}; extra_stats = {}; self.skip_newlines()
        while not self.check("DEDENT"):
            if self.check("ID") and self.peek().value == "STATS":
                self.advance(); stats, extra_stats = self.parse_stats_block()
            else:
                fname = self.expect("ID").value; self.expect("="); fexpr = self.parse_expr(); self.expect("NEWLINE"); fields[fname] = fexpr
            self.skip_newlines()
        self.expect("DEDENT")
        return {"type": "HomebrewDef", "kind": kind, "name": name, "fields": fields, "stats": stats, "extra_stats": extra_stats}

    def parse_stats_block(self):
        self.expect(":"); self.expect("NEWLINE"); self.expect("INDENT")
        stats = {}; extra_stats = {}; self.skip_newlines()
        while not self.check("DEDENT"):
            if self.check("extra"):
    ...
    def parse_summon(self):
        self.expect("summon"); var_name = self.expect("ID").value; self.expect("="); template_name = self.expect("ID").value
        self.expect("("); self.expect(")"); self.expect("NEWLINE")
        return {"type": "SummonDecl", "var_name": var_name, "template": template_name}

    def parse_while(self):
        self.expect("while"); condition = self.parse_expr()
        return {"type": "While", "condition": condition, "body": self.parse_block()}
    ...
    def parse_for(self):
        ...
        while self.check(","):
            self.advance(); args.append(self.parse_expr())
        self.expect(")")
        return {"type": "ForParty", "var": var_name, "args": args, "body": self.parse_block()}
    ...
    def parse_shift_spec(self):
        if self.peek().type in ("+", "-"):
            sign = self.advance().type; num_tok = self.expect("NUMBER"); value = num_tok.value if sign == "+" else -num_tok.value
            return {"type": "ShiftLiteral", "value": value}
        if self.check("NUMBER"):
            return {"type": "ShiftLiteral", "value": self.advance().value}
        name = self.expect("ID").value
        if name in ("mod", "number") and not self.check("."):
            return {"type": "ShiftStatRef", "stat": None, "mode": name}
        if self.check("."):
            self.advance(); mode = self.expect("ID").value
        ...
        while True:
            if self.check("."):
                ...
                if field == "force" and self.check("."):
                    node = self.parse_force_call(instance=node); continue
                if field == "sway" and self.check("("):
                    node = self.parse_sway_call(instance=node); continue
                node = {"type": "FieldAccess", "obj": node, "field": field}
            elif self.check("("):
                self.advance(); args = []
                if not self.check(")"):
    ...
    def parse_primary(self):
        ...
        if tok.type == "player":
            self.advance(); self.expect("("); prompt = self.parse_expr(); self.expect(")"); return {"type": "PlayerInput", "prompt": prompt}
...
def _attach_statement_locations(parser, statements):
    """Attach the token location to every executable AST statement."""
    for stmt in statements:
        # parse_statement wrapper is responsible for locations; this helper is
        # retained for callers that may build an AST manually.
        if "address" not in stmt:
            stmt["address"] = None
    return statements
