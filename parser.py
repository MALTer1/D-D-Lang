class Parser:
 ...
 def parse_assignment_or_expr(self):
  ...
  if self.check('='):self.advance();v=self.parse_expr();self.expect('NEWLINE');return {'type':'Assignment','target':t,'value':v}
  if self.check('+='):self.advance();r=self.parse_expr();self.expect('NEWLINE');return {'type':'Assignment','target':t,'value':{'type':'BinaryOp','op':'+','left':t,'right':r}}
